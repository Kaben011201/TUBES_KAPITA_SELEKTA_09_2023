...
const ActivityScreen = () => {
  ...
  const getActivity = async () => {
    try {
      const response = await axiosConfig.get("api/activity", {
  ...
  const deleteData = async (activity) => {
    let text = "Apakah mau hapus?";
    if (confirm(text) == true) {
      await delActivityHapus(activity); // Wait for the asynchronous function to complete
    }
  };

  const delActivityHapus = async (activity) => {
    try {
      const response = await axiosConfig.delete(`api/activity/${activity.id}`);
      if (response.data.status !== 400) {
  ...
  const getActivityEdit = async (activity) => {
    try {
      const response = await axiosConfig.get(`api/activity/${activity.id}`);
      if (response.data.status !== 400) {
      } else {
        alert(response.data.message);
      }
          ...
          <td className="whitespace-nowrap border-0">
            ...
            <dialog id="modal_edit_activity" className="modal">
              <div className="modal-box xl:max-w-7xl">
                ...
                <form
                  onSubmit={patchActivityEdit}
                  className="flex flex-col gap-[7px] text-xs xl:text-base my-2 mt-6 whitespace-normal"
                >
                  ...
                  <div className="flex gap-3 xl:gap-4 items-center">
                    ...
                    >
                      Keterangan/ Kegiatan
                      <span className="text-red-500 absolute mt-[-20px]">
                        *
                      </span>
                    </label>
                    <input
                  ...
                  <button
                    ...
                    name="tambah"
                    id="tambah"
                    className="bg-[#FF5757;] w-[120px] xl:w-[220px] xl:h-[50px] h-[35px] self-end mt-3 rounded-[10px] xl:rounded-[15px] text-white font-semibold text-sans text-xs xl:text-base p-2 xl:p-5 flex items-center gap-1 xl:gap-3"
                  >
                    <svg
      ...
      <section className="mt-[10px] xl:mt-[20px] w-[85%] mx-auto xl:border-2 xl:px-7 xl:py-5 xl:rounded-xl">
        <div className="flex items-center p-[15px] text-xs xl:text-base justify-end">
          <button
            ...
            id="inputKegiatan"
            className="bg-[#FF5757;] w-fit xl:h-[50px] h-[25px] rounded-[5px] xl:rounded-[15px] text-white font-semibold text-sans xl:text-base p-2 xl:p-5 flex items-center gap-1 xl:gap-3"
          >
            + Input Data Kegiatan
          </button>
        </div>
